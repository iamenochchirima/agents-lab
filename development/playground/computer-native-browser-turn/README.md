# Playground slice: Computer Native browser turn

## Question

What does the real Computer Native browser path look like when a model reads a local
...
The automated browser tests remain under `computer-native/tests/`.
...
The normal saved `computer-native/.env` configuration is used. The fixture server
prints the URLs if you choose a different port with
`COMPUTER_NATIVE_BROWSER_FIXTURE_PORT`.
