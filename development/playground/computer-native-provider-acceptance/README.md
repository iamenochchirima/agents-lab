# Computer Native real-provider acceptance

This is a short manual check for the standalone Computer Native provider path. It uses
...
From `computer-native/`, copy `.env.example` to `.env` and set:
...
```bash
cd /home/enoch/aworkspace/agents/agents-lab/computer-native
pnpm run chat
```
