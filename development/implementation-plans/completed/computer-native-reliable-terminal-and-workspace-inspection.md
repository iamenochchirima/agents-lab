...
- [x] Add a bounded `computer-native doctor` or equivalent provider-check command that
...
- `node development/playground/computer-native-terminal-turn/run.mjs` — passed; normal, tool, rejected-path, provider-failure, and interrupted-recovery paths observed
