...
Add the first usable Skills module to Anesu. A skill is a local, reusable
...
- Anesu keeps the same separation. The Skills module owns discovery and
...
Implemented in the standalone Anesu runtime:
...
- `node --test --test-concurrency=1 dist/tests/skills.test.js dist/tests/approval-tui.test.js`
  — 18 tests passed.
- `pnpm test` — the full Anesu suite completed with no reported failures.
