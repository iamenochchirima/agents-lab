...
- `pnpm --filter @agent-harness-lab/anesu run typecheck` — passed.
- `pnpm --filter @agent-harness-lab/anesu test` — 179 passed, 0 failed.
- The browser-focused build and tests — 57 passed, 0 failed.
- `pnpm --filter @agent-harness-lab/anesu coverage` — 179 passed, 0 failed;
...
- [artifacts ownership](../../../anesu/src/artifacts/README.md)
- [telemetry ownership](../../../anesu/src/telemetry/README.md)
...
- [completed process execution](../completed/anesu-process-execution.md)
...
Add the first real browser capability to Anesu. The agent must be able to
...
From `anesu/`, this real flow works with the configured model:
...
text-entry action. Anesu shows the exact action and approval context, waits
...
- [x] Create an isolated browser profile for each Anesu browser session and
...
| Local reference | Pattern used | Anesu decision |
| --- | --- | --- |
| OpenClaw `extensions/browser` | Dedicated browser capability with explicit profiles, lifecycle, routing, tab identity, SSRF checks, upload/download handling, and browser-specific tests. | Keep browser implementation under a standalone Anesu browser boundary. Start with one managed local profile and preserve a future adapter seam without adding remote control now. |
...
- Do not let a browser library define Anesu's approval or evidence contract.
- Do not interpret page instructions as Anesu policy. Page content is untrusted
...
- [x] Add Playwright as a direct, pinned Anesu dependency only if the adapter
...
- [x] Add browser ownership and lifecycle documentation under `anesu/`.
...
- [x] Document Playwright/browser-binary setup, security limitations, approval rules,
      and manual checks.
- [x] Add a small `development/playground/anesu-browser-turn/` inspection
...
- [x] From `anesu/`, run `pnpm run chat` with the configured real model.
...
pnpm --filter @agent-harness-lab/anesu run build
git diff --check
```
...
- `pnpm --filter @agent-harness-lab/anesu coverage` — 179 passed, 0 failed;
  package-wide line coverage reported 87.61%.
- `pnpm --filter @agent-harness-lab/anesu run build` — passed.
