# Anesu core hardening and production foundation
...
- [CLI ownership](../../../anesu/src/cli/README.md)
- [Workspace ownership](../../../anesu/src/workspace/README.md)
- [Anesu package commands](../../../anesu/package.json)
...
- [Completed process execution plan](../completed/anesu-process-execution.md)
- [Completed filesystem plan](../completed/anesu-workspace-filesystem.md)
- [Completed browser plan](../completed/anesu-browser-interaction.md)
- [Completed memory plan](../completed/anesu-memory.md)

These references inform boundaries and failure semantics. They do not make Computer
Native equivalent to either project. Anesu must keep its own contracts,
evidence, and tests.

## Purpose

Close the foundational gaps that would make the next Anesu capabilities
...
production-readiness gap register for reinforcement after more Anesu
...
- The retry policy is configurable through `ANESU_MODEL_RETRY_ATTEMPTS` and
  `ANESU_MODEL_RETRY_BACKOFF_MS`.
...
- Multi-file patch preparation now uses an explicit aggregate resulting-content budget,
  ...
  `ANESU_MAX_PATCH_SET_BYTES`; directory-tree operations retain their separate
...
Anesu into a general durable workflow system.
...
- This is the smallest useful integrity check for the model evidence Anesu
...
- [ ] Update Anesu documentation and the follow-on queue so the next slice
...
- [x] Add operation-specific aggregate mutation limits for large inputs: directory-tree
      ...
      now enforce `ANESU_MAX_PATCH_SET_BYTES` over resulting member content
...
whole Anesu product are production-ready.
