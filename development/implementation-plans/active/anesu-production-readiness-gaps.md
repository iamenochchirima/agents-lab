# Anesu production-readiness gaps
...
This document is the source of truth for what remains before Anesu can be
...
verified. They do not mean that the whole Anesu product is mature. The honest
current status is:

> Anesu has real local foundations for model turns, workspace actions,
...
This register covers the standalone Anesu product. It does not require the
...
- Persisted workspace mutation records are now schema-validated before writes, transitions,
...
- Model requests are rejected before provider transport when their serialized size
  exceeds `ANESU_MAX_MODEL_REQUEST_BYTES`; streamed response text and tool-call
  fields are bounded by `ANESU_MAX_MODEL_OUTPUT_BYTES` and fail without a
...
focus after more Anesu components are integrated:
...
Every Anesu area moves through the same ladder:
...
- The first aggregate mutation limits are now implemented: directory-tree operations use
  ...
  `ANESU_MAX_PATCH_SET_BYTES` resulting-content cap before approval and commit.
...
but Anesu does not yet have a production skill or plugin system.
...
Current state: Anesu is primarily a bounded foreground turn runner. Durable
...
Current state: the standalone Anesu TUI is the current product surface. The
main Agent Harness Lab UI does not yet consume a complete Anesu session,
...
Anesu may be called production-ready only when all of these statements are
...
- [Anesu completed plans](../completed/README.md)
...
- [Anesu memory plan](../completed/anesu-memory.md)
- [Anesu process plan](../completed/anesu-process-execution.md)
- [Anesu browser plan](../completed/anesu-browser-interaction.md)
- [Anesu filesystem plan](../completed/anesu-workspace-filesystem.md)
