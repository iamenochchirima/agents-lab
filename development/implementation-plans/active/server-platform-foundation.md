...
- [completed Temporal implementation](../completed/lab-server-temporal-baseline.md)
...
- [ ] Generalize the run-domain types without erasing useful platform-specific data.
- [ ] Move platform-specific manifest settings behind the selected runner seam.
- [ ] Replace the Temporal-only execution reference with a generic platform execution
      reference that can carry safe native details.
- [ ] Generalize evidence-store native reference handling and allowlisted evidence
      paths while preserving safe path rules.
- [ ] Make `RunService` reconcile, cancel, and project runs through the generic runner
      interface only.
- [ ] Make health and platform availability data represent registered runners rather
      than one hardcoded platform.
- [ ] Migrate the Temporal adapter and its tests without changing Temporal semantics.
- [ ] Add a test-only second runner and runner conformance tests.
- [ ] Add a reusable platform implementation-plan template with explicit ownership,
      dependencies, tests, evidence, documentation, and commit boundaries.
- [ ] Update server and Platform UI documentation where response or evidence contracts
      change.
- [ ] Record exact validation results, manual checks, release impact, and known limits
...
- [ ] List every Temporal-specific field in common domain, application, HTTP, evidence,
      bootstrap, UI, and tests.
- [ ] Decide the generic manifest shape and platform-owned configuration seam.
- [ ] Decide the generic execution-reference shape and native redaction rules.
- [ ] Decide how old `native/temporal.json` evidence is read during migration.
- [ ] Decide whether health returns one generic runner map or a list of runner statuses.
- [ ] Record compatibility changes as an ADR or an explicit section in this plan.
- [ ] Commit this checkpoint before delegating implementation work.

### 2. Common domain and runner seam

- [ ] Generalize `RunManifest`, request validation, and manifest creation.
- [ ] Preserve immutable effective configuration and safe serialization.
- [ ] Rename or replace workflow-shaped common types with platform-neutral names.
- [ ] Keep lifecycle statuses common and document platform-specific status mapping.
- [ ] Keep the runner interface small: validation, connectivity, start, inspect, cancel.
- [ ] Document start retry, cancellation, inspection, and unknown-outcome semantics.
- [ ] Add runner conformance fixtures without putting fake platform behavior in production.

### 3. Evidence and reconciliation

- [ ] Generalize `RunEvidenceSnapshot` and `RunView` away from `temporalReference`.
- [ ] Add safe native reference read/write by platform identifier.
- [ ] Keep evidence filenames allowlisted and reject traversal or unknown native paths.
- [ ] Preserve event identity, source ordering, idempotent writes, and terminal-result rules.
- [ ] Keep platform-specific event detail available without requiring it in normalized files.
- [ ] Test server restart/reconciliation using a generic runner reference.
- [ ] Test missing references, unavailable runners, duplicate events, and conflicting evidence.

### 4. Temporal migration

- [ ] Move namespace, task queue, workflow type, activity names, and Temporal connection
      rules behind the Temporal adapter.
- [ ] Keep Temporal workflow IDs and duplicate-start recovery unchanged.
- [ ] Keep query, signal, cancellation, retry, and worker-restart behavior unchanged.
- [ ] Keep the fake and OpenRouter model paths unchanged.
- [ ] Keep Temporal native evidence safe and inspectable.
- [ ] Update Temporal tests to use the generic reference only at the server seam.
- [ ] Run the local Temporal integration test with a real local Temporal server.

### 5. Health, registry, and HTTP compatibility

- [ ] Make registry entries the source of truth for runnable and planned variants.
- [ ] Make health report registered runner connectivity without hardcoding one platform.
- [ ] Keep errors actionable and prevent native SDK errors from leaking credentials.
- [ ] Update API response types and UI client types together if response fields change.
- [ ] Verify the existing Temporal Platform UI can still submit and inspect a run.
- [ ] Do not show a planned platform as runnable merely because its directory exists.

### 6. Reusable platform plan template

- [ ] Add a template for future platform implementation plans.
- [ ] Require platform identity, variant identity, language/runtime, and local dependency.
- [ ] Require adapter ownership and files allowed to change.
- [ ] Require execution model, durability model, state model, and platform-native evidence.
- [ ] Require normal completion, failure, cancellation, retry, restart, and unknown-outcome
      semantics.
- [ ] Require unit, integration, local-infrastructure, manual, and UI acceptance checks.
- [ ] Require documentation updates, release impact, commit boundaries, and known limits.
- [ ] Require a parallel-agent handoff section that names safe file ownership.

### 7. Documentation and learning path

- [ ] Update the server architecture diagram to show platform adapters behind one seam.
- [ ] Document what the common server guarantees and what it deliberately does not normalize.
- [ ] Document how a new platform plan is created and how an adapter becomes runnable.
- [ ] Update the Temporal architecture and local-development guides.
- [ ] Update Platform UI documentation if native reference fields or health responses change.
- [ ] Add a short comparison table showing common run records versus native platform records.
- [ ] Keep this plan focused on the server platform layer, not Computer Native.

## Test coverage

### Unit tests

- [ ] generic manifest validation and immutable serialization
- [ ] platform and variant lookup, planned versus runnable status
- [ ] runner contract validation and safe error mapping
- [ ] generic execution-reference persistence
- [ ] native evidence path allowlisting and platform-name validation
- [ ] event identity, source ordering, duplicate handling, and terminal idempotency
- [ ] run service dispatch, inspection, cancellation, and reconciliation with a test runner
- [ ] Temporal-specific configuration remains inside the Temporal adapter

### Integration tests

- [ ] existing Temporal baseline run completes through the generic server path
- [ ] existing Temporal cancellation still produces the same terminal semantics
- [ ] Temporal worker restart recovery still projects one normalized result
- [ ] a generic test runner completes and fails through the same HTTP/run service path
- [ ] server restart reconciles a retained generic execution reference
- [ ] unavailable platform execution does not fabricate a result
- [ ] unknown or planned platform requests remain rejected with a clear response
- [ ] native evidence for two runner identities cannot overwrite each other

### Manual acceptance checks

- [ ] Start local Temporal and `./scripts/run_local_stack.sh`.
- [ ] Run a Temporal fake-model request from the Platform UI.
- [ ] Inspect `config.json`, `events.jsonl`, `trajectory.json`, `metrics.json`,
      `result.json`, and `native/temporal.json`.
- [ ] Stop and restart the server while a Temporal run is retained, then inspect it again.
- [ ] Confirm a planned platform is visible as planned but cannot be run.
- [ ] Exercise the generic test adapter through its test harness, not the production UI.
- [ ] Confirm no API key, raw provider header, or SDK credential appears in evidence or logs.

## Documentation, release, and commit checklist

- [ ] Update relevant server and platform documentation in the same change.
- [ ] Record analytics decision: not applicable unless new product interaction metrics are added.
- [ ] Record structured logging decision: preserve safe runner/platform identifiers only.
- [ ] Record metrics decision: keep normalized run metrics; add no invented platform metrics.
- [ ] Record version/release identity decision: manifest server version remains populated.
- [ ] Record rollout decision: local-only change; no deployment rollout required.
- [ ] Record rollback decision: revert the foundation commits while retaining old Temporal evidence.
- [ ] Record known limitations and migration rules in the completion record.
- [ ] Use focused commits rather than one large implementation commit:
...
- [ ] Move this plan to `completed/` only after exact validation output and manual checks
      are recorded.
