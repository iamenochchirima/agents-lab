...
function parseValue(value: string): string {
  ...
  if (trimmed.length >= 2 && trimmed.startsWith('"') && trimmed.endsWith('"')) {
    try {
      ...
      throw new ComputerNativeError("configuration", "The local .env file contains an invalid quoted value.");
    }
  }
  if (trimmed.length >= 2 && trimmed.startsWith("'") && trimmed.endsWith("'")) {
...
export async function loadLocalEnvironment(
  ...
  try {
    ...
    throw new ComputerNativeError("configuration", `Could not read local environment file '${path.basename(filePath)}'.`, { cause: error });
  }
  ...
  for (const [index, rawLine] of contents.split(/\r?\n/u).entries()) {
    ...
    if (!match) {
      throw new ComputerNativeError("configuration", `The local .env file has an invalid entry on line ${index + 1}.`);
    }
