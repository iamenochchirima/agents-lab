...
export function browserStartFailureMessage(error: unknown): string {
  ...
  if (/executable doesn't exist|browserType\.launch|playwright install/iu.test(detail)) {
    return "Managed Chromium is unavailable. Install it with `pnpm --filter @agent-harness-lab/computer-native exec playwright install chromium`.";
  }
 ...
 * pages, but deliberately exposes only the Computer Native browser contracts.
 */
