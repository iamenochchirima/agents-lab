...
import { DEFAULT_MAX_TREE_BYTES, DEFAULT_SEARCH_MAX_MATCHES, MAX_QUARANTINE_ENTRIES, MAX_SEARCH_MATCHES, type PreparedWorkspaceMutation, type Workspace } from "../workspace/workspace.js";
...
export class ToolRegistry {
  ...
  private async executePreparedMutation(call: ModelToolCall, prepared: PreparedWorkspaceMutation, context: ToolExecutionContext, mutationId = `mutation_${randomUUID().replaceAll("-", "")}`): Promise<ToolExecutionResult> {
    ...
    const request = {
      ...
      ...(prepared.operation === "delete-directory-tree" || prepared.operation === "restore-directory" ? { manifestHash: prepared.manifestHash, entryCount: prepared.entryCount, totalBytes: prepared.totalBytes, maxBytes: this.workspace.policy.limits.maxTreeBytes ?? DEFAULT_MAX_TREE_BYTES, maxDepth: prepared.maxDepth } : prepared.operation === "copy" || prepared.operation === "move" || prepared.operation === "rename" ? prepared.kind === "directory" ? { manifestHash: prepared.manifestHash, entryCount: prepared.entryCount, totalBytes: prepared.bytes, maxBytes: this.workspace.policy.limits.maxTreeBytes ?? DEFAULT_MAX_TREE_BYTES, maxDepth: prepared.maxDepth } : {} : {}),
