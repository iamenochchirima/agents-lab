...
test("session lock records an owner identity and rejects a live owner", async () => {
  const root = await mkdtemp(path.join(os.tmpdir(), "computer-native-lock-"));
  try {
    ...
    await assert.rejects(
      ...
      (error: unknown) => error instanceof ComputerNativeError && error.code === "lock" && /already in use/u.test(error.message),
    );
    await lock.release();
  } finally {
...
test("session lock removes a live-pid lock when its Linux start identity is stale", { skip: process.platform !== "linux" }, async () => {
  const root = await mkdtemp(path.join(os.tmpdir(), "computer-native-lock-stale-"));
  try {
    const lockPath = path.join(root, "session", ".lock");
...
test("session lock keeps backwards-compatible PID-only ownership conservative", async () => {
  const root = await mkdtemp(path.join(os.tmpdir(), "computer-native-lock-legacy-"));
  try {
    const lockPath = path.join(root, "session", ".lock");
    await mkdir(path.dirname(lockPath), { recursive: true });
...
test("session lock does not treat a non-positive PID as the current process group", async () => {
  const root = await mkdtemp(path.join(os.tmpdir(), "computer-native-lock-invalid-pid-"));
  try {
    const lockPath = path.join(root, "session", ".lock");
    await mkdir(path.dirname(lockPath), { recursive: true });
...
test("session lock does not delete partially published metadata", async () => {
  const root = await mkdtemp(path.join(os.tmpdir(), "computer-native-lock-partial-"));
  try {
    const lockPath = path.join(root, "session", ".lock");
    await mkdir(path.dirname(lockPath), { recursive: true });
    ...
    await assert.rejects(
      ...
      (error: unknown) => error instanceof ComputerNativeError && error.code === "lock" && /metadata is invalid/u.test(error.message),
    );
