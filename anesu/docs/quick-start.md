...
`ANESU_MAX_PATCH_SET_BYTES` (256 KiB by default), and journals each member with
...
10,000 entries per journal. Set `ANESU_MEMORY_EVIDENCE_RETENTION_DAYS` or
`ANESU_MEMORY_EVIDENCE_MAX_ENTRIES` in `.env` to change those bounds; prepared
deletion evidence is retained and maintenance fails closed if the safe retained set is
larger than the configured bound.

When `ANESU_PROCESS_MODE=approval` (the development default), the model also
has `run_command`. It accepts an executable and exact `args` array, not a shell command
string. In an interactive TTY, Anesu shows the executable, argument vector,
...
sandbox; use `ANESU_PROCESS_MODE=deny` to remove the capability entirely.
...
`ANESU_BROWSER_SESSION_TIMEOUT_MS`. Expiry closes the managed browser and
...
`ANESU_BROWSER_ALLOWED_LOCAL_HOSTS` when needed. Unsafe schemes, embedded
...
[Anesu browser playground](../../development/playground/anesu-browser-turn/README.md):
...
configured limits. Set `ANESU_MODEL_RETRY_ATTEMPTS` and
`ANESU_MODEL_RETRY_BACKOFF_MS` in `.env` to adjust that policy. A failure after
...
512 KiB (`ANESU_MAX_MODEL_REQUEST_BYTES`) and the default streamed response
limit is 256 KiB (`ANESU_MAX_MODEL_OUTPUT_BYTES`). A request above its limit is
