import { AnesuError, isAbortError, ModelProviderError, redactSecrets } from "../runtime/errors.js";
  ...
  async *stream(request: ModelRequest, signal: AbortSignal): AsyncIterable<ModelStreamEvent> {
    ...
    const countResponseBytes = (value: string): void => {
      ...
      if (responseBytes > this.maxOutputBytes) {
        throw new AnesuError("resource-limit", `The model response exceeded the ${this.maxOutputBytes}-byte limit.`);
      }
    };
    try {
      while (true) {
      ...
      if (error instanceof AnesuError || isAbortError(error)) throw error;
