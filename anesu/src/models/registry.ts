...
function validateModelSelection(provider: ProviderName, model: string): void {
  ...
  if (parts.length < 2 || parts.some((part) => part.trim().length === 0) || /\s/u.test(model)) {
    if (provider === "openrouter") {
      throw new AnesuError("configuration", `OpenRouter model '${model}' must be a namespaced OpenRouter model such as nvidia/model:free or openrouter/free.`);
    }
    throw new AnesuError("configuration", `The deterministic model must start with deterministic/; received '${model}'.`);
  }
...
const descriptors: readonly ModelProviderDescriptor[] = [
  ...
  },
  {
    ...
    create: (config) => {
      if (!config.openRouterApiKey) throw new AnesuError("configuration", "OpenRouter credentials are unavailable.");
...
export function getModelProviderSummary(provider: ProviderName): ModelProviderSummary {
  ...
  if (!summary) throw new AnesuError("configuration", `Unsupported provider '${provider}'.`);
  return summary;
}
...
export function createModelProvider(config: AppConfig): ModelProvider {
  ...
  if (!descriptor) throw new AnesuError("configuration", `Unsupported provider '${config.provider}'.`);
