...
untrusted-content marker; instructions in a page cannot change Anesu policy.
...
`ANESU_BROWSER_READ_RETRY_COUNT`. Navigation and all side-effecting actions
...
with `ANESU_BROWSER_PROFILE_RETENTION_MS`,
`ANESU_BROWSER_ARTIFACT_RETENTION_MS`, and
`ANESU_BROWSER_CLEANUP_MAX_ENTRIES`; `doctor` prints their effective values.
...
tool result keeps the typed Anesu outcome and does not expose raw adapter
details.
