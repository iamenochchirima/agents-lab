...
const MEMORY_START = "<!-- anesu-memory: ";
const MEMORY_END = "<!-- /anesu-memory -->";
...
export class MemoryPolicyError extends AnesuError {
...
export class MemoryStore {
  ...
  async add(mutation: MemoryMutation): Promise<MemoryRecord> {
    ...
    if (!created) throw new AnesuError("persistence", "Memory was not created.");
  ...
  async flushBeforeCompaction(_request: MemoryFlushRequest, _signal?: AbortSignal): Promise<MemoryFlushResult> {
    this.assertOpen();
    return { status: "not-supported", recordsWritten: 0, reason: "Context compaction is not owned by the Anesu memory slice." };
  }
  ...
  async replace(input: {
    ...
    if (!replaced) throw new AnesuError("persistence", "Memory was not replaced.");
  ...
  private async canonicalFileHash(filePath: string): Promise<string> {
    ...
    if (relative.startsWith("..") || path.isAbsolute(relative)) throw new AnesuError("persistence", "Memory recovery referenced a path outside the managed memory directory.");
    try {
      const information = await lstat(filePath);
      if (information.isSymbolicLink() || !information.isFile()) throw new AnesuError("persistence", "Memory recovery referenced a non-regular canonical file.");
      ...
      throw new AnesuError("persistence", "Could not inspect the canonical memory file during recovery.", { cause: error });
    }
  }
  ...
  private assertOpen(): void {
    if (this.closed) throw new AnesuError("persistence", "Memory store is closed.");
  }
}
...
async function openDerivedDatabase(databasePath: string): Promise<DatabaseSync> {
  try {
    ...
    if (information.isSymbolicLink() || !information.isFile()) throw new AnesuError("persistence", "The durable memory index must be a regular file.");
  } catch (error) {
  ...
  try {
    ...
    try {
      ...
      throw new AnesuError("persistence", "The durable memory index is corrupt and could not be moved aside for rebuild.", { cause: renameError });
    }
    try {
      ...
      throw new AnesuError("persistence", "The durable memory index could not be rebuilt from canonical memory.", { cause: rebuildError ?? error });
    }
  }
}
...
async function readCanonicalRecords(memoryDir: string): Promise<StoredMemoryRecord[]> {
  ...
  for (const file of files) {
    ...
    try {
      ...
      if (information.isSymbolicLink() || !information.isFile()) throw new AnesuError("persistence", `Memory file '${path.basename(file.filePath)}' must be a regular file.`);
      content = await readFile(file.filePath, "utf8");
    } catch (error) {
      if ((error as NodeJS.ErrnoException).code === "ENOENT") continue;
      throw new AnesuError("persistence", `Could not read memory file '${path.basename(file.filePath)}'.`, { cause: error });
    }
  ...
  while (cursor < content.length) {
    ...
    if (start < 0) {
      if (content.slice(cursor).trim().length > 0 && !content.slice(cursor).trim().startsWith("#")) {
        throw new AnesuError("persistence", `Memory file '${path.basename(filePath)}' contains unmarked content.`);
      }
      break;
    }
    ...
    if (metadataEnd < 0 || end < 0) throw new AnesuError("persistence", `Memory file '${path.basename(filePath)}' contains an incomplete entry.`);
    ...
    if (metadata.scope !== expectedScope || !/^[a-f0-9]{64}$/u.test(metadata.contentHash)) {
      throw new AnesuError("persistence", `Memory file '${path.basename(filePath)}' failed its integrity check.`);
    }
    try {
      ...
      throw new AnesuError("persistence", `Memory file '${path.basename(filePath)}' contains invalid entry metadata.`, { cause: error });
    }
...
function decodeMetadata(encoded: string): MemoryFileMetadata {
  try {
    ...
    throw new AnesuError("persistence", "A memory entry has invalid metadata.", { cause: error });
  }
}
...
async function atomicWriteText(filePath: string, content: string): Promise<void> {
  ...
  try {
    ...
    throw new AnesuError("persistence", `Could not replace memory file '${path.basename(filePath)}'.`, { cause: error });
  }
}
...
async function ensureManagedDirectory(directory: string): Promise<void> {
  try {
    ...
    if (!information.isDirectory() || information.isSymbolicLink()) {
      throw new AnesuError("persistence", `Memory path '${path.basename(directory)}' must be a private real directory.`);
    }
  } catch (error) {
    if ((error as NodeJS.ErrnoException).code !== "ENOENT") throw error;
...
async function readRepairableJsonLines<T>(filePath: string, isValid: (value: unknown) => value is T): Promise<{ readonly values: T[]; readonly repairedTrailingLine: boolean }> {
  ...
  try {
    ...
    throw new AnesuError("persistence", `Could not read memory evidence '${path.basename(filePath)}'.`, { cause: error });
  }
  ...
  for (const line of completeLines) {
    ...
    if (!isValid(value)) throw new AnesuError("persistence", `Memory evidence '${path.basename(filePath)}' contains invalid evidence.`);
    values.push(value);
  }
...
function parseEvidenceLine(line: string, filePath: string): unknown {
  try {
    ...
    throw new AnesuError("persistence", `The memory evidence '${path.basename(filePath)}' contains malformed JSON.`, { cause: error });
  }
}
