...
import { AnesuError, redactSecrets } from "../runtime/errors.js";
...
export async function readJsonLines<T>(filePath: string): Promise<T[]> {
  ...
  try {
    ...
    throw new AnesuError("persistence", `Could not read '${path.basename(filePath)}'.`, { cause: error });
  }
...
export function safePathSegment(value: string, label: string): string {
  if (!/^[A-Za-z0-9][A-Za-z0-9_-]{0,127}$/u.test(value)) {
    throw new AnesuError("invalid-input", `${label} contains unsupported characters.`);
  }
  return value;
}
