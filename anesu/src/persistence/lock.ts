  ...
  static async acquire(filePath: string, options: { readonly waitMs?: number } = {}): Promise<SessionLock> {
    ...
    try {
      ...
      if ((error as NodeJS.ErrnoException).code !== "EEXIST") {
        throw new AnesuError("lock", "Could not acquire the session lock.", { cause: error });
      }
      ...
      try {
        ...
        throw new AnesuError("lock", "The existing session lock could not be inspected.", { cause: readError });
      }
      ...
      try {
        ...
        throw new AnesuError("lock", "The existing session lock metadata is invalid; refusing to remove it.", { cause: parseError });
      }
      ...
      if (ownerPid !== undefined) {
        ...
        try {
          ...
          if (probeError instanceof AnesuError) throw probeError;
          if ((probeError as NodeJS.ErrnoException | undefined)?.code === "EPERM") {
            throw new AnesuError("lock", `Session owner process ${ownerPid} exists but could not be probed.`, { cause: probeError });
          }
        }
      }
      await rm(filePath, { force: true });
