...
import { AnesuError, isRuntimeInterruptionError, ModelProviderError, redactSecrets, RuntimeInterruptionError, safeErrorMessage } from "./errors.js";
import type {
  ModelMessage,
  ModelToolCall,
  TurnMetrics,
  ...
  try {
    ...
    for (let round = 1; round <= options.config.maxModelToolRounds; round += 1) {
      ...
      if (requestBytes > options.config.maxModelRequestBytes) {
        ...
        throw new AnesuError("resource-limit", `The model request is ${requestBytes} bytes, above the ${options.config.maxModelRequestBytes}-byte limit.`);
      }
      let attempt = 0;
      while (true) {
        attempt += 1;
        ...
        try {
          ...
          await turn.appendEvent("ModelAttemptCompleted", {
            ...
            errorCode: error instanceof AnesuError ? error.code : "provider",
            reason,
            requestBytes,
            maxRequestBytes: options.config.maxModelRequestBytes,
            responseBytes: roundOutputBytes,
