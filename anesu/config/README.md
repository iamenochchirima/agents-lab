...
Anesu configuration belongs here: model settings, workspace policy, tool
policy, and security controls. Do not put Lab experiment configuration in this
directory.

The first terminal slice reads `ANESU_*` settings and the optional
...
Workspace mutations are bounded independently: `ANESU_MAX_FILE_BYTES` limits
one file, `ANESU_MAX_PATCH_SET_BYTES` limits the resulting bytes across one
multi-file patch set, and `ANESU_MAX_TREE_BYTES` limits one directory-tree
