...
test("restart marks a running browser action ambiguous without replaying it", async () => {
  const stateDir = await mkdtemp(path.join(os.tmpdir(), "anesu-browser-records-"));
  try {
...
test("restart repairs a missing first browser lifecycle event", async () => {
  const stateDir = await mkdtemp(path.join(os.tmpdir(), "anesu-browser-first-event-recovery-"));
  try {
    const session = await SessionStore.open(stateDir);
...
test("restart repairs a missing browser prelude before an already durable terminal event", async () => {
  const stateDir = await mkdtemp(path.join(os.tmpdir(), "anesu-browser-prelude-recovery-"));
  try {
    const session = await SessionStore.open(stateDir);
...
test("recovery rejects a malformed browser action before classifying it", async () => {
  const stateDir = await mkdtemp(path.join(os.tmpdir(), "anesu-browser-malformed-record-"));
  try {
    const session = await SessionStore.open(stateDir);
...
test("browser tool turns persist lifecycle phases and redact configured secrets", async () => {
  const stateDir = await mkdtemp(path.join(os.tmpdir(), "anesu-browser-turn-"));
  const workspaceRoot = await mkdtemp(path.join(os.tmpdir(), "anesu-browser-workspace-"));
  const artifactRoot = await mkdtemp(path.join(os.tmpdir(), "anesu-browser-turn-artifacts-"));
  try {
    const session = await SessionStore.open(stateDir);
...
test("diagnostic interruption after a browser action does not repeat it", async () => {
  const stateDir = await mkdtemp(path.join(os.tmpdir(), "anesu-browser-interruption-state-"));
  const workspaceRoot = await mkdtemp(path.join(os.tmpdir(), "anesu-browser-interruption-workspace-"));
  const artifactRoot = await mkdtemp(path.join(os.tmpdir(), "anesu-browser-interruption-artifacts-"));
  try {
    const session = await SessionStore.open(stateDir);
    const workspace = await Workspace.open(workspaceRoot, { maxFileBytes: 4_096, maxDirectoryEntries: 20 });
    const urlPolicy = new BrowserUrlPolicy({ allowedLocalHosts: ["127.0.0.1"], dnsLookup: async () => ["127.0.0.1"] });
...
test("browser completion acknowledgement loss preserves an uncertain outcome without replay", async () => {
  const stateDir = await mkdtemp(path.join(os.tmpdir(), "anesu-browser-ack-boundary-state-"));
  const workspaceRoot = await mkdtemp(path.join(os.tmpdir(), "anesu-browser-ack-boundary-workspace-"));
  const artifactRoot = await mkdtemp(path.join(os.tmpdir(), "anesu-browser-ack-boundary-artifacts-"));
  try {
    let actionWrites = 0;
...
test("browser interruption before the durable start record does not launch the action", async () => {
  const stateDir = await mkdtemp(path.join(os.tmpdir(), "anesu-browser-pre-start-state-"));
  const workspaceRoot = await mkdtemp(path.join(os.tmpdir(), "anesu-browser-pre-start-workspace-"));
  const artifactRoot = await mkdtemp(path.join(os.tmpdir(), "anesu-browser-pre-start-artifacts-"));
  try {
    let actionWrites = 0;
    const session = await SessionStore.open(stateDir, undefined, {
      writeHooks: {
