...
test("browser artifact store writes bounded screenshot metadata under its managed root", async () => {
  const root = await mkdtemp(path.join(os.tmpdir(), "anesu-browser-artifacts-"));
  try {
...
test("browser artifact store rejects oversized screenshots and removes the file", async () => {
  const root = await mkdtemp(path.join(os.tmpdir(), "anesu-browser-artifacts-limit-"));
  try {
...
test("browser artifact store rejects screenshots over the configured pixel bounds", async () => {
  const root = await mkdtemp(path.join(os.tmpdir(), "anesu-browser-artifacts-dimensions-"));
  try {
...
test("browser artifact finalization rejects symlink targets and mismatched observations", async () => {
  const root = await mkdtemp(path.join(os.tmpdir(), "anesu-browser-artifacts-finalize-"));
  const outside = await mkdtemp(path.join(os.tmpdir(), "anesu-browser-artifacts-finalize-outside-"));
  try {
...
test("browser artifact paths reject symlinked managed directories", async () => {
  const root = await mkdtemp(path.join(os.tmpdir(), "anesu-browser-artifacts-directory-"));
  const outside = await mkdtemp(path.join(os.tmpdir(), "anesu-browser-artifacts-directory-outside-"));
  try {
...
test("browser artifact cleanup removes expired pairs and orphans while retaining recent artifacts", async () => {
  const root = await mkdtemp(path.join(os.tmpdir(), "anesu-browser-artifacts-cleanup-"));
  const outside = await mkdtemp(path.join(os.tmpdir(), "anesu-browser-artifacts-outside-"));
  try {
...
test("browser artifact cleanup removes an expired temporary metadata publication", async () => {
  const root = await mkdtemp(path.join(os.tmpdir(), "anesu-browser-artifacts-metadata-temp-"));
  try {
...
test("browser artifact cleanup stops at its candidate bound", async () => {
  const root = await mkdtemp(path.join(os.tmpdir(), "anesu-browser-artifacts-bound-"));
  try {
...
test("browser artifact cleanup retains an in-flight artifact with a live lease", async () => {
  const root = await mkdtemp(path.join(os.tmpdir(), "anesu-browser-artifacts-lease-"));
  try {
    const store = new BrowserArtifactStore(root);
...
test("browser artifact cleanup retains an expired artifact with corrupt ownership metadata", async () => {
  const root = await mkdtemp(path.join(os.tmpdir(), "anesu-browser-artifacts-corrupt-lock-"));
  try {
...
test("browser upload policy accepts bounded regular workspace files and rejects links or traversal", async () => {
  const root = await mkdtemp(path.join(os.tmpdir(), "anesu-browser-upload-"));
  try {
