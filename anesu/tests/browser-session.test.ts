...
test("orphaned profile cleanup removes only old managed profiles within the explicit root", async () => {
  const root = await mkdtemp(path.join(os.tmpdir(), "anesu-browser-profile-cleanup-"));
  const outside = await mkdtemp(path.join(os.tmpdir(), "anesu-browser-profile-outside-"));
  try {
...
test("orphaned profile cleanup stops at its entry bound", async () => {
  const root = await mkdtemp(path.join(os.tmpdir(), "anesu-browser-profile-bound-"));
  try {
...
test("orphaned profile cleanup retains a profile with a live ownership lease", async () => {
  const root = await mkdtemp(path.join(os.tmpdir(), "anesu-browser-profile-lease-"));
  try {
