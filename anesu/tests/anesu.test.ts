...
import { AnesuError, ModelProviderError, MutationError, RuntimeInterruptionError } from "../src/runtime/errors.js";
...
beforeEach(async () => {
  temporaryDirectories.push(await mkdtemp(path.join(os.tmpdir(), "anesu-test-")));
});
...
test("configuration has safe deterministic defaults and rejects missing OpenRouter credentials", () => {
  ...
  const configuredMemoryEvidence = loadConfig({ stateDir: tempDirectory() }, {
    ANESU_MEMORY_EVIDENCE_RETENTION_DAYS: "14",
    ANESU_MEMORY_EVIDENCE_MAX_ENTRIES: "250",
  });
...
test("local environment loading preserves explicit variables and reads development settings", async () => {
  ...
  await writeFile(envPath, "ANESU_PROVIDER=openrouter\nOPENROUTER_MODEL='openrouter/free'\nLOCAL_ONLY=from-file\n", "utf8");
  const environment = await loadLocalEnvironment(envPath, {
    ANESU_PROVIDER: "deterministic",
    EXISTING: "preserved",
  });
  assert.equal(environment.ANESU_PROVIDER, "deterministic");
...
test("timeout and cancellation produce distinct terminal results", async () => {
  ...
  const cancelDir = await mkdtemp(path.join(os.tmpdir(), "anesu-cancel-"));
...
test("OpenRouter adapter rejects a response stream above its configured byte limit", async () => {
  ...
  await assert.rejects(
    ...
    (error: unknown) => error instanceof AnesuError && error.code === "resource-limit" && error.message.includes("4-byte limit"),
  );
});
...
test("OpenRouter adapter classifies incomplete streams and rate limits", async () => {
  ...
  await assert.rejects(
    ...
    (error: unknown) => error instanceof AnesuError && error.code === "rate-limit" && !error.message.includes("secret"),
  );
...
test("OpenRouter adapter rejects malformed stream shapes as non-retryable provider errors", async () => {
  ...
  const malformedField = new OpenRouterModelProvider("openai/example", "secret", async () => new Response(malformedFieldStream, { status: 200 }));
  await assert.rejects(
    async () => { for await (const _event of malformedField.stream(request, new AbortController().signal)) void _event; },
    (error: unknown) => error instanceof ModelProviderError && error.code === "provider-incomplete" && error.retryable === false,
  );

  const malformedIndexStream = new ReadableStream<Uint8Array>({
    start(controller) {
      controller.enqueue(new TextEncoder().encode('data: {"choices":[{"delta":{"tool_calls":[{"index":-1,"function":{"name":"read_file","arguments":"{}"}}]}}]}\n\n'));
      controller.close();
    },
  });
  const malformedIndex = new OpenRouterModelProvider("openai/example", "secret", async () => new Response(malformedIndexStream, { status: 200 }));
  await assert.rejects(
    async () => { for await (const _event of malformedIndex.stream(request, new AbortController().signal)) void _event; },
    (error: unknown) => error instanceof ModelProviderError && error.code === "provider-incomplete" && error.retryable === false,
  );

  const malformedIdStream = new ReadableStream<Uint8Array>({
    start(controller) {
      controller.enqueue(new TextEncoder().encode('data: {"choices":[{"delta":{"tool_calls":[{"index":0,"id":7,"function":{"name":"read_file","arguments":"{}"}}]}}]}\n\n'));
      controller.close();
    },
  });
  const malformedId = new OpenRouterModelProvider("openai/example", "secret", async () => new Response(malformedIdStream, { status: 200 }));
  await assert.rejects(
    async () => { for await (const _event of malformedId.stream(request, new AbortController().signal)) void _event; },
    (error: unknown) => error instanceof ModelProviderError && error.code === "provider-incomplete" && error.retryable === false,
  );

  const malformedNameStream = new ReadableStream<Uint8Array>({
    start(controller) {
      controller.enqueue(new TextEncoder().encode('data: {"choices":[{"delta":{"tool_calls":[{"index":0,"function":{"name":7,"arguments":"{}"}}]}}]}\n\n'));
      controller.close();
    },
  });
  const malformedName = new OpenRouterModelProvider("openai/example", "secret", async () => new Response(malformedNameStream, { status: 200 }));
  await assert.rejects(
    async () => { for await (const _event of malformedName.stream(request, new AbortController().signal)) void _event; },
    (error: unknown) => error instanceof ModelProviderError && error.code === "provider-incomplete" && error.retryable === false,
  );
});
...
test("OpenRouter adapter rejects malformed streamed text fields as non-retryable provider errors", async () => {
  const request: ModelRequest = {
    sessionId: asSessionId("session_test"),
    turnId: asTurnId("turn_test"),
    provider: "openrouter",
  ...
  const malformedChunks = [
    ...
    '{"choices":[{"delta":{"refusal":{"message":"no"}}}]}',
  ];
...
test("workspace quarantines a file for deletion and restores it from the recovery token", async () => {
  ...
  assert.equal(deleted.quarantinePath, ".anesu-trash/mutation_delete_test/payload");
  ...
  const invalidEntry = path.join(root, ".anesu-trash", "mutation_invalid_entry");
  ...
  const manifestPath = path.join(root, ".anesu-trash", "mutation_delete_test", "manifest.json");
  ...
  await assert.rejects(() => workspace.stat(".anesu-trash"), /reserved for internal recovery data/);
  await assert.rejects(() => workspace.stat(".anesu-transactions"), /reserved for internal recovery data/);
...
test("workspace quarantines, restores, and purges a bounded directory tree with a manifest", async () => {
  ...
  await assert.rejects(() => workspace.commitQuarantinePurge(partialPurge), (error: unknown) => error instanceof AnesuError && error.code === "reconciliation-required");
...
test("workspace rejects a cross-device move before approval", async () => {
  ...
  const destinationRelative = `proc/anesu-move-${path.basename(sourceRoot)}`;
  await assert.rejects(
...
`,
  ...
  assert.deepEqual(committed.journal.members.map((member) => member.temporaryPath), [
    ".anesu-transactions/mutation_patch_set_test/member-1.tmp",
    ".anesu-transactions/mutation_patch_set_test/member-2.tmp",
  ]);
  ...
  await assert.rejects(() => lstat(path.join(root, ".anesu-transactions", "mutation_patch_set_test")), { code: "ENOENT" });
  assert.equal(await readFile(path.join(root, "one.txt"), "utf8"), "one new\n");
  ...
  const exposedTransactionRoot = path.join(exposedRoot, ".anesu-transactions");
...
test("workspace rejects a multi-file patch set above its aggregate resulting-byte limit", async () => {
  ...
  await assert.rejects(() => lstat(path.join(root, ".anesu-transactions")), { code: "ENOENT" });
});
...
`,
  ...
  assert.deepEqual(latestJournal.members.map((member) => ({ path: member.path, state: member.state, temporaryPath: member.temporaryPath })), [
    { path: "one.txt", state: "committed", temporaryPath: ".anesu-transactions/mutation_patch_set_partial/member-1.tmp" },
    { path: "two.txt", state: "staged", temporaryPath: ".anesu-transactions/mutation_patch_set_partial/member-2.tmp" },
  ]);
  assert.equal((await lstat(path.join(root, ".anesu-transactions", "mutation_patch_set_partial"))).isDirectory(), true);
...
test("multi-file recovery distinguishes no commit from all members committed before journal acknowledgement", async () => {
  ...
  await assert.rejects(() => lstat(path.join(beforeRoot, ".anesu-transactions", "mutation_patch_set_before_first")), { code: "ENOENT" });
  ...
  await assert.rejects(() => lstat(path.join(afterRoot, ".anesu-transactions", "mutation_patch_set_after_all")), { code: "ENOENT" });
});
...
test("deterministic model can inspect a file through the bounded tool loop", async () => {
  ...
  const root = path.join(stateDir, "workspace");
  await mkdir(root, { recursive: true });
  await writeFile(path.join(root, "README.txt"), "Anesu", "utf8");
  const session = await openSession(stateDir);
  const workspace = await Workspace.open(root, { maxFileBytes: 100, maxDirectoryEntries: 10 });
...
`,
  ...
  assert.equal(mutation.journal.transactionPath, `.anesu-transactions/${mutation.mutationId}`);
...
test("model delete_directory_tree flow persists bounded manifest metadata and a restore token", async () => {
  ...
  assert.match(mutation.quarantinePath ?? "", /^\.anesu-trash\/mutation_[A-Za-z0-9]+\/payload$/);
...
test("model delete flow persists the quarantine path and restore token", async () => {
  ...
  assert.equal(mutation.status, "committed");
  assert.match(mutation.quarantinePath ?? "", /\.anesu-trash/);
